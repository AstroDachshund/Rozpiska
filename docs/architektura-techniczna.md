...
16. **CRUD zasobów trenera = RSC + Server Actions (nie TanStack Query)** (rozstrzygnięte w S3.1, M3). Bank ćwiczeń: lista to Server Component czytający przez klienta SSR (RLS zawęża do trenera), mutacje to Server Actions z Zod + `revalidatePath('/exercises')`. TanStack Query celowo NIE wchodzi tu — provider pojawi się dopiero w M4 (kreator, drzewo planu) i M6 (cache offline klienta), gdzie cache/optimistic mają realną wartość. Tagi: kolor partii mięśniowej z palety talerzy w czystej funkcji `lib/domain/tag-colors.ts` (design-system §5.6); pozostałe kategorie neutralne. Multi-write (ćwiczenie + linki tagów) w akcji sekwencyjnie, bez RPC — niskie ryzyko, częściowa awaria naprawialna edycją (inaczej niż template→assignment w M5, który MUSI być atomowy). Brak migracji i nowych testów RLS (schemat + izolacja z M1). **Lekcja (final review S3.1):** dodając nowe trasy grupy `(trainer)`/`(client)` TRZEBA dopisać je do `TRAINER_PREFIXES`/`CLIENT_PREFIXES` w `lib/auth/routes.ts` — sama grupa tras `(trainer)` to tylko motyw, nie autoryzacja; egzekwuje ją middleware ról (S2.3). `/exercises` początkowo pominięte → traktowane jako publiczne (naprawione).

17. **Wyszukiwarka ćwiczeń (Command ⌘K) — globalna w (trainer), kontrolowany komponent reużywalny pod M4** (rozstrzygnięte w S3.2, M3). `components/exercise-command.tsx` nie zarządza własnym stanem otwarcia ani skrótem klawiszowym (propsy `open`/`onOpenChange`/`onSelect`) — to granica, którą M4 wykorzysta z innym `onSelect` (wstawienie ćwiczenia do sekcji planu, zamiast nawigacji do edycji). Globalne okablowanie na dziś (`components/trainer-command-menu.tsx`, ⌘K w całym `(trainer)`, nawigacja do edycji) mieszka osobno. Filtr tagów: semantyka AND w czystej funkcji `lib/domain/exercise-filter.ts`; wyszukiwanie tekstowe — **własny `filter` na `Command`, nie domyślny `cmdk`**, bo domyślny nie składa polskich diakrytyków (find w final review: „wioslowanie" nie trafiało w „Wiosłowanie hantlem"). Ponieważ shadcn-owe `CommandDialog` nie przekazuje dodatkowych propsów (`...props` trafia na `Dialog`, nie na wewnętrzny `Command`), `exercise-command.tsx` **nie używa `CommandDialog`** — zamiast tego składa `Dialog`+`DialogContent`+`Command` wprost (te same, niemodyfikowane eksporty z `components/ui/*`), replikując dokładnie strukturę `CommandDialog` (klasy, `sr-only` `DialogTitle`/`DialogDescription`), żeby dopiąć `filter` do właściwego elementu. „Widok zarchiwizowanych" to toggle WEWNĄTRZ palety (drugi zestaw już pobranych danych), nie osobna strona. `app/(trainer)/layout.tsx` pobiera ćwiczenia+tagi raz na nawigację przez istniejące `lib/exercises/queries.ts` (`Promise.all`) — bez nowej warstwy cache (TanStack Query nadal odłożone do M4/M6, decyzja §9.16); cały bank ćwiczeń trenera trafia teraz do payloadu KAŻDEJ strony trenera (nie tylko `/exercises`) — świadomy kompromis przy małych zbiorach w MVP, do rewizji przy stałym sidebar nav / większych bankach. `Command`/`Dialog` shadcn dodane przez oficjalny generator (`npx shadcn add command`), bez modyfikacji generowanych plików.

18. **Warstwa danych kreatora: hooki TanStack Query nad istniejącym drzewem M1, pozycja float dołożenia-na-końcu** (rozstrzygnięte w S4.1, M4). Zero migracji — schemat + RLS + triggery dziedziczenia właściciela (`plan_weeks_inherit_ownership`, `plan_structure_inherit_ownership`) istnieją od M1. `QueryClientProvider` wpięty w root layout TERAZ (nie odłożony do S4.2) — hooki są od razu w pełni funkcjonalne. Kolumny właścicielskie (`trainer_id`/`client_id`/`template_id`/`assigned_plan_id`) nigdy nie idą z klienta — trigger je wypełnia z rodzica; wygenerowany typ `Insert` oznacza `trainer_id` jako wymagane (kolumna `NOT NULL`), więc payloady insertów są rzutowane (`as Database[...]['Insert']`) — bezpieczne i udokumentowane w każdym pliku mutacji. `lib/domain/position.ts`: `positionBetween`/`rebalance`, odstęp startowy `STEP=1000`, próg degeneracji `1e-7`. **Lekcja z review Task 5**: mutacje create tylko DOKŁADAJĄ na końcu rodzeństwa (`positionBetween(ostatnia, null)`), co strukturalnie nigdy się nie degeneruje (gałąź `after === null` w `positionBetween` zwraca wynik zanim w ogóle sprawdzi próg) — pierwotna implementacja miała martwą gałąź `if (position === null) { rebalance… }` skopiowaną do wszystkich pięciu plików mutacji; złapane w review, usunięte z Tasków 5–7 i z samego dokumentu planu. `rebalance` zostaje w `position.ts` (przetestowany) pod przyszłe wstawianie MIĘDZY sąsiadami (reorder/dnd, S4.2/S4.4), gdzie `positionBetween(before, after)` faktycznie może zwrócić null. Odczyt drzewa: jedno zagnieżdżone zapytanie Supabase + czyste, testowane mapowanie/sortowanie po stronie klienta na każdym poziomie (nie przez nested-order API Supabase — prościej, niezależne od wersji). Delete węzłów struktury to twardy DELETE (bez `archived_at` w schemacie) — bezpieczne, bo `set_logs.plan_set_id` ma `ON DELETE SET NULL` (historia nigdy nie ginie). Hooki bez testów jednostkowych (brak jsdom w projekcie, jak w S3.2) — zweryfikuje je S4.2, gdy UI zacznie ich realnie używać.

19. **PlanBuilderTree — edytor drzewa tygodnie→dni→sekcje nad warstwą danych S4.1** (rozstrzygnięte w S4.2, M4, bez prototypu wizualnego D3 — nie powstał; zbudowane wprost z docs/design-system.md §5.5 za zgodą Piotra). UI: `components/plan-builder/{plan-builder-tree,week-node,day-node,section-row,inline-editable-text}.tsx` + trasy `(trainer)/templates` (lista/tworzenie) i `(trainer)/templates/[id]` (edytor). Zero nowego kodu mutującego — cały UI woła bezpośrednio hooki z S4.1. Kluczowe decyzje:
    ...
    - **Tworzenie węzłów bez formularza**: tydzień auto-numerowany (`max(week_number)+1`, NIE po długości tablicy — odporne na luki), dzień z placeholderem „Dzień N" od razu w trybie inline-rename. Oba korzystają wprost z append-only ścieżki mutacji S4.1 (`positionBetween(last, null)`).
    - **Usuwanie przez natywny `confirm()`** — bez `AlertDialog`; kaskada usuwa całe poddrzewo (schemat: `ON DELETE CASCADE` na FK drzewa).
    - **Guard `disabled={mutation.isPending}` na każdym przycisku dodawania** (sekcja/dzień/tydzień) — dodane w pre-flight tego planu po lekturze final review S4.1, który odnotował wyścig: dwa szybkie kliknięcia czytają tę samą, jeszcze nieodświeżoną pozycję z cache przed `onSettled`, tworząc dwa rodzeństwa o tej samej `position`/`week_number`. Guard per-przycisk wystarcza na ten konkretny przypadek; pełne UI optymistyczne odłożone.
